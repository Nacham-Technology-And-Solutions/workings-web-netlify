...
```ts
{
  materialList: MaterialListItem[];   // Profiles, glass sheets
...
|-----------|----------------------|--------|
| `materialList` where `type === 'Profile'` | **Profiles** | Stock bar counts (`units`) |
...
| Accessories | Handles, friction stays, stoppers only for **`O`** panels, not all `N`. |
