...
| Value | Layout |
| ----- | ------ |
| `Two_Glass_Panel` | 2-track, 2 glass panels |
| `Three_Glass_Panel` | 3-track, 3 glass panels |
| `Two_Glass_And_One_Net_Panel` | 3-track, 2 glass + 1 sliding net |

### `fixedNet` (optional)

Boolean (or `options.fixedNet`). When `true`, adds a **fixed net panel** on top of any `panel` layout (1132-Net Profile, Net Mesh, Net Angle, Net Rubber, Attachment Screws).

### Points

Derived from `panel` + `fixedNet` — see [CALCULATION-MODULES-PARAMETERS.md](api/CALCULATION-MODULES-PARAMETERS.md#unified-sliding-window-m2m5-family).

### Backward compatibility

- Legacy **`sash`** field and `*_Sash` enum values (`Two_Glass_Sash`, etc.) still accepted.
- Legacy module IDs `M2_Sliding_2Sash` … `M5_Sliding_3Sash` resolve to `Sliding_Window` + `panel` + `fixedNet`.

### Profile name change

2-track jamb is **`2-Track Jamb`** (was `Jamb Profile` on legacy M2 output).

---

## 5. Module 3 — Sliding 2-panel + fixed net (legacy)

Same unified calculator as §4b with `panel: "Two_Glass_Panel"` and `fixedNet: true`. Legacy carts:

- **`M3_Sliding_2Sash_Net`** / **`M3_Sliding_2Sash_FixedNet`**
- **`M2_Sliding_2Sash`** with `options: { fixedNet: true }` or top-level `"fixedNet": true`

All resolve to the same engine path.
...
| `rubberTotals`    | **Wool Pile / Brush** | metres (from 2-glass sliding panels)                 |
...
## 7. Sliding windows (`Sliding_Window` + legacy M2–M5)

### Two glass panels (`Two_Glass_Panel`, `fixedNet: false`) — legacy M2
...
### Two glass + fixed net (`Two_Glass_Panel`, `fixedNet: true`) — legacy M3
...
### 3-track layouts — legacy M4 & M5

| Layout / `panel` | Rollers (calc)   | Purchase display                                                         | Net                                                                 |
| ---------------- | ---------------- | ------------------------------------------------------------------------ | ------------------------------------------------------------------- |
| **`Two_Glass_And_One_Net_Panel`** (M4) | **6 pcs/window** | **`Rollers`**, `set` + `pieceQty` (6 sets for 24 pcs on regression cart) | `netList.cuts` (panes) + **`materialList`** `Net Mesh Roll` (rolls) |
| **`Three_Glass_Panel`** (M5) | **6 pcs/window** | same as M4                                                               | no `netList` (all glass)                                            |

**M4 layout (`Two_Glass_And_One_Net_Panel`) accessories (per window, × cart qty):**

| `accessoryTotals.name`   | `unit` | Per window            |
| ------------------------ | ------ | --------------------- |
| `Key/lock`               | `sets` | 2 (glass panels only) |
| `Track Screw (Long 4mm)` | `pcs`  | 8                     |
| `Coupling Screw`         | `pcs`  | 12 (3 panels × 4)     |

**M4 layout `rubberTotals`:** `Glazing Rubber` (glass panes only), `Wool Pile / Brush` (3 sliding panels).

**M5 layout (`Three_Glass_Panel`) accessories (per window, × cart qty):**
...
- `panel` — **`Sliding_Window`** layout: `Two_Glass_Panel` | `Three_Glass_Panel` | `Two_Glass_And_One_Net_Panel` (legacy: `sash` / `*_Sash` values).
- `fixedNet` or `options.fixedNet` — optional fixed net panel; applies to **any** `panel` type.
