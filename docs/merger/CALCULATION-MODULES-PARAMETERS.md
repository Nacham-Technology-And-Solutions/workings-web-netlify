...
| `M5_Sliding_3Sash`     | 3-sash all-glass (legacy alias)           | Windows  |
...
**Description:** Single calculator for all sliding window layouts. Legacy module IDs (`M2_Sliding_2Sash` … `M5_Sliding_3Sash`) still work and map to `sash` + `fixedNet` automatically.
...
| `sash`      | string  | Sash layout | Required — see enum below |
...
| ---------- | ------- | ----------- |
| `fixedNet` | boolean | Add fixed net panel (any `sash` type). Also accepts `options.fixedNet` for backward compatibility. |

### `sash` enum

| Value | Meaning |
| ----- | ------- |
| `Two_Glass_Sash` | 2-track, 2 glass sashes (legacy M2) |
| `Three_Glass_Sash` | 3-track, 3 glass sashes (legacy M5) |
| `Two_Glass_And_One_Net_Sash` | 3-track, 2 glass + 1 sliding net (legacy M4) |
...
| ------------------ | ----------- |
| `M2_Sliding_2Sash` | `Two_Glass_Sash`, `fixedNet: false` (or `options.fixedNet`) |
| `M3_Sliding_2Sash_Net` | `Two_Glass_Sash`, `fixedNet: true` |
| `M4_Sliding_3Track` | `Two_Glass_And_One_Net_Sash`, `fixedNet: false` |
| `M5_Sliding_3Sash` | `Three_Glass_Sash`, `fixedNet: false` |

### Points cost

Derived from `sash` + `fixedNet`: M2=5, M3=8, M4=8, M5=10; fixed net adds +3 on any sash type.
