...
Legacy `M2_Sliding_2Sash` … `M5_Sliding_3Sash` carts still work. Prefer **`Sliding_Window`** with `panel` + `fixedNet` for new integrations.

### Sliding `panel` values

| `panel` | Typical use |
| ------- | ------------- |
| `Two_Glass_Panel` | 2-track, 2 glass |
| `Three_Glass_Panel` | 3-track, 3 glass |
| `Two_Glass_And_One_Net_Panel` | 3-track, 2 glass + 1 sliding net |

Set `"fixedNet": true` to add a fixed net panel on any layout (+3 calculation points).

### Sliding profile `itemKey`s (estimation)

| Profile | `itemKey` |
| ------- | --------- |
| Track Profile | `profile.track` |
| 2-Track Jamb | `profile.jamb.2track` |
| 3-Track Profile | `profile.track.3` |
| 1-Track Jamb | `profile.jamb.1track` |
| Lock Stile | `profile.lock_stile` |
| Rollers | `accessory.rollers` |
| Lockset/Key | `accessory.lockset` |

Legacy name **`Jamb Profile`** maps to `profile.jamb.2track`.
