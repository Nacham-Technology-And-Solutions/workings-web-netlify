...
const SettingItem: React.FC<SettingItemProps> = ({ icon, label, isActive, onClick }) => (
  <button
    onClick={onClick}
    className={`w-full flex items-center text-left p-4 rounded-lg transition-colors ${isActive
        ? 'bg-gray-100 text-gray-900'
        : 'text-gray-700 hover:bg-gray-50'
      }`}
    aria-label={`Go to ${label} settings`}
  >
    <div className={`mr-3 ${isActive ? 'text-gray-900' : 'text-gray-600'}`}>
      {icon}
    </div>
...
const SettingsScreen: React.FC<SettingsScreenProps> = ({ onMenuClick, onNavigate }) => {
  const [activeSection, setActiveSection] = useState<'profile' | 'billings' | 'subscriptionPlans'>('profile');
