...
interface ProjectMeasurementScreenProps {
  ...
  /** Called when Recalculate is confirmed - triggers fresh calculation (clears cached result) */
  onRecalculate?: (data: ProjectMeasurementData) => void;
  previousData?: SelectProjectData;
  /** Existing dimensions/unit when returning from Calculation Results */
  initialMeasurementData?: ProjectMeasurementData | null;
  onNavigateToStep?: (step: string) => void;
  /** When true, primary action shows "Recalculate" (e.g. after "Modify dimensions & recalculate") */
  isRecalculate?: boolean;
}

const ProjectMeasurementScreen: React.FC<ProjectMeasurementScreenProps> = ({ onBack, onNext, onRecalculate, previousData, initialMeasurementData, onNavigateToStep, isRecalculate }) => {
      ...
      <div className="px-4 md:px-8 py-4 md:py-6 border-b border-gray-100">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="flex items-center gap-2 text-sm text-gray-400 mb-6">
            ...
            <span>/</span>
            <span className="cursor-pointer hover:text-gray-600 transition-colors" onClick={() => { onNavigateToStep ? onNavigateToStep('projectDescription') : onBack(); }}>Project Description</span>
            <span>/</span>
            <span className="cursor-pointer hover:text-gray-600 transition-colors" onClick={() => { onNavigateToStep ? onNavigateToStep('selectProject') : onBack(); }}>Glazing Category</span>
            <span>/</span>
          ...
          <div className="flex items-start justify-between gap-4">
            <div className="flex items-start gap-4 flex-1 min-w-0">
              ...
              <div>
                <h1 className="text-lg md:text-2xl font-bold text-gray-900 mb-1">Select Glazing Type</h1>
            ...
            <div className="hidden md:flex items-center gap-3 flex-shrink-0">
              {isRecalculate && onNavigateToStep && (
                <button
                  ...
                  className="px-5 py-2.5 font-semibold rounded transition-colors bg-white border border-gray-300 text-gray-700 hover:bg-gray-50"
                >
      ...
      {showRecalculateConfirm && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-[9999]">
          <div className="bg-white rounded-lg p-6 max-w-md w-full mx-4 shadow-xl">
            ...
            <h3 className="text-lg font-semibold text-gray-900 mb-2 text-center">Recalculate project?</h3>
            <p className="text-sm text-gray-600 mb-6 text-center">This will cost additional points.</p>
            <div className="flex gap-3">
              <button
                onClick={() => setShowRecalculateConfirm(false)}
                className="flex-1 px-4 py-3 border border-gray-300 text-gray-700 text-sm font-semibold rounded hover:bg-gray-50 transition-colors"
              >
                Cancel
              </button>
              <button
                onClick={handleRecalculateConfirm}
                className="flex-1 px-4 py-3 bg-gray-900 text-white text-sm font-semibold rounded hover:bg-gray-800 transition-colors"
              >
                Recalculate
              </button>
            </div>
              ...
              <div className="bg-white border border-gray-200 rounded-lg p-6">
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    className={`flex-1 py-3 rounded font-medium transition-colors flex items-center justify-center gap-2 ${isFormValid
                  ...
                  {editingId && (
                    <button
                      ...
                      className="px-4 py-3 rounded font-medium border border-gray-300 text-gray-700 hover:bg-gray-50 transition-colors"
                    >
                      Cancel
                    </button>
                  )}
